...
\new{The $repOK$ property is written using JAVA code and is a predicate (a function that returns a Boolean value) representing the class invariants
...
The $repOK$ predicate method will check that the LinkedList class doesn't have any cycles and that the number of nodes traversed from root matches the value of the field size.
...
\new{These invariants could not be inferred in an automatic form, so it must be written by the developer or the person in charge for the verification.
