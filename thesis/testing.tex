...
\ac{SBGT}, Constraint-based Generation Testing, Grammar-based Generation Testing and
...
The different categories of software testing enables the detection of faults of the system from different perspectives and in differents stages
of software development.
In certain situations, because there are few scenarios or low criticality of the system,
certain categories of testing can be planned together. Acording to \cite{citeulike:453686} the following are the most frequent:
\begin{description}
\item[Unit Testing] Unit tests are designed to test small parts or
units of the system, such as methods or small pieces of code;
\item[Component Testing] The target universe of this type of test is a little larger than the
test unit, here it is important to test the components as a whole, but not considering the interaction with other parts of the system;
\item[Integration Testing] Tests are looking for flaws from the integration of system components, usually related to sending and receiving data;
\item[System Testing] In this type of test to search for errors is performed using the system as would be done by the end user;
...
The most widely known are the white-box and black-box.
...
like \ac{SB}, Constraint-based, Grammar-based and Random-based tests generation
...
needed the implementation to start the development of test cases. The only thing needed is the functional contracts (The specification, or a model)
...
These specifications serve as input for the definition of test cases to test a given system.
...
namely that exist a gap between the level of abstraction of a model and level of abstraction of test cases.
...
\textbf{Definition of derivation criteria of test cases from the model}: Must first establish a technique to be used in the design of the test cases,
...
The derivation criteria defines the test coverage. For example in \cite{Lindholm06model-basedtesting}, where is generated test cases from \ac{FSA},
...
\textbf{Generation of test cases and test suite}: The derivation of test criteria defined in the previous section are applied.
From this interaction test cases are generated and grouped into test suites.
\textbf{Adaptation of test cases}: 
...
These specifications typically do not consider structurally complex inputs and the existing tools do not generate JUnit test cases.
