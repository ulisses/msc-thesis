...
recent tools that uses: Specification-based testing and
...
generation techniques. Furthermore we will show the potential of the described tools.
...
Concerning the first case we use a simple example based on a function that uses a multiplication, creating a simple branche on the code.
\begin{code}
typedef struct s {
    int x;
    int y;
...
\end{code}
We have tried to use pointers instead of coping the structure as a parameter to $Multiply$ function, but PathCrawler was not able to run.
...
\end{code}
We have used a function that PathCrawler give to us: $pathcrawler\_assert$, this function can be used at any location in the
...
The results we got were interesting: 31 covered paths and 44 infeasible paths and the test as interrupted by PathCrawler,
...
do the assertion in line 8. We did not written any pre-condition, so PathCrawler does not know that this is a pre-condition
...
Because C\# is a more expressive language than C we will improve our examples with some other OO and C\# specific features like Exceptions and Debug.Assert calls.
...
So, as we describe earlier, Pex will try to generate such input as it is possible (in a given amount of time) to traverse all the paths inside the code.
...
First we have defined the properties about this data structure. The most relevant ones are property \ref{eq:linked} in Figure \ref{fig:formulae} that
...
We took properties in Fig. \ref{fig:formulae} and use them to restrict the generation of structures using Java. So we define the $repOK$ method that receives
a $LinkedList$ structure and returns $Bool$ whenever this structure follows the invariants in \ref{fig:formulae}.
