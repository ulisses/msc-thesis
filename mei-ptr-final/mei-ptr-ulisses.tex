...
testing and then will be discussed in more detail some automatic
generation techniques. Furthermore the potential of the described tools will be explained, and how they can help
...
Here will be presented a study on the most recent tools that uses: Specification-based or Constraint-based tests generation
approaches for the most popular languages - C, JAVA and C\#.
...
Concerning the first case will be used a simple example based on a function that uses a multiplication, creating a simple branch on the code.
\begin{code}
typedef struct s {
    int x;
    int y;
