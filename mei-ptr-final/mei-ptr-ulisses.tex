...
With this motivation in mind, this paper explores automatic testing approaches and tools in order to propose a system that satisfies with VST needs.
...
\subsection{Goals}
This document correspond to the first milestone in the author's MSc. Dissertation, trying to produce
a tool that is able to automatically generate interesting testcases for the C++ ESA's Operational Simulator.\\
Here will be shown reviews of the most studied techniques
...
generation techniques. Furthermore the potential of the described tools will be explained, and how can they help
solve the problem VST has nowadays.
...
After introducing the theory and the techniques that support each tool will be demonstrated some of the tools in action, resorting to small but illustrative examples
...
to provide information to feedback loop\cite{Xie03mutuallyenhancing} generation testing framework with two test prespectives: functional and structural and from there
be able to get a more refined set of tests.\\
The combination of both symbolic execution from Pex and complex data generation from Korat will be developed
to generate more interesting inputs for the methods under testing.

\section{Conclusion}\label{Concl}
Looking for an efficient solution to automatically generate complete test sets for complex and critical C++ software, the state-of-the-art approaches in the area were studied and along the paper some tools were introduced from methodological and experimental perspectives.
...
We have discussed some of the tested tools and present a mixture between some of them to be able to generate a more suitable
...
For that purpose, Frama-C will be explored, as it is well known that this tool is able to infer pre- and postconditions\cite{moy} and interesting safety conditions from C source code.
